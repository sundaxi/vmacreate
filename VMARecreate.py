...
class GetParmeters(object):
    ...
    def GetParmeters(self):
        ...
        if az_show_vm_json[_storageProfile][_osDisk].get(_managedDisk) is not None:
            ...
            az_temp_vm_attach_disk_cmd = "az vm disk attach --disk %s --resource-group %s --vm-name %s" % (vm_os_disk,self.vm_resourcegroup,self.vm_temp_name)
            az_temp_vm_detach_disk_cmd = "az vm disk detach -g %s --vm-name %s -n %s" % (self.vm_resourcegroup,self.vm_temp_name,vm_os_disk)
            if len(self.vm_os_datadisk_list) > 0:
                for datadisk in self.vm_os_datadisk_list:
                    ...
                    az_vm_attach_data_disk = "az vm disk attach --disk %s --resource-group %s --vm-name %s --caching %s\n" % (datadisk[_managedDisk][_id].split("/")[-1], self.vm_resourcegroup, self.vm_name, data_caching)
            ...
            az_storage_blob_copy = "az storage blob copy start -u %s -b %s -c %s --account-name %s" %(vm_vhd, vhd_name.split(".vhd")[0] + "_bak.vhd",datename,storageaccount)
            az_bak_disk_uri = storageaccount_uri + "/" + datename + "/" + vhd_name.split(".vhd")[0] + "_bak.vhd"
            ...
            az_temp_vm_attach_disk_cmd = "az vm unmanaged-disk attach --resource-group %s --vm-name %s --vhd-uri %s -n %s" % (self.vm_resourcegroup, self.vm_temp_name, vm_vhd ,vhd_name.split(".vhd")[0])
            az_temp_vm_detach_disk_cmd = "az vm unmanaged-disk detach -g %s --vm-name %s -n %s" % (self.vm_resourcegroup,self.vm_temp_name,vhd_name.split(".vhd")[0])
            ...
            if len(self.vm_os_datadisk_list) > 0:
                for datadisk in self.vm_os_datadisk_list:
                    if datadisk[_caching] is None:
                        data_caching = "None"
                    data_caching = datadisk[_caching]
                    az_vm_attach_data_disk = "az vm unmanaged-disk attach --resource-group %s --vm-name %s --vhd-uri %s -n %s --caching %s\n" % (self.vm_resourcegroup,self.vm_name, datadisk[_vhd][_uri], datadisk[_vhd][_uri].split("/")[-1].split('.vhd')[0],data_caching)
                    az_vm_attach_data_disk_cmd = az_vm_attach_data_disk + az_vm_attach_data_disk_cmd
            else:
                az_vm_attach_data_disk_cmd = "No data disk"
        ...
        return az_cmd_generated, az_backup_cmd, az_vm_create_temp_cmd, az_temp_vm_attach_disk_cmd, az_temp_vm_detach_disk_cmd, az_vm_attach_data_disk_cmd
